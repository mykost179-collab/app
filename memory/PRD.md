...
- Fitur Unduh JPG (2026-09-25, revisi 2): tombol "Share" (ikon unduh) + pemilih tata letak dua ikon ponsel di atasnya — portrait (kalender di atas, legenda di bawah, output 2560px skala 4) & landscape (kalender di kiri, legenda di kanan, output 3840px 4K); Share terkunci (disabled) sampai salah satu gaya dipilih; render canvas native 2D per orientasi di `lib/exportPoster.js` (drawPortrait/drawLandscape); share sheet native di ponsel, fallback unduh di desktop; html2canvas hanya fallback darurat
- Simbol Baru Dipakai (2026-09-25): ikon yang baru dipakai muncul di baris "BARU DIPAKAI" di atas pemilih simbol (localStorage mydate-recent-icons, maks 10, tampil 8)
- Ingat Pilihan (2026-09-25): orientasi poster (portrait/landscape) tersimpan di localStorage (mydate-orientation) — Share langsung aktif saat kunjungan berikutnya
- Penanda Berulang (2026-09-25): pilihan Sekali/Mingguan/Bulanan + jumlah 4/8/12 di form penanda baru; salinan dibuat frontend (mingguan +7 hari, bulanan clamp ke akhir bulan) lalu dikirim sebagai batch POST; toast menghitung total penanda
