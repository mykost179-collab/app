...
- Fitur Unduh JPG (2026-09-25, revisi): tombol "Share" (ikon unduh) merender poster via canvas native 2D (bukan html2canvas) di `lib/exportPoster.js` — digambar matematis pada 3840px (4K UHD) sehingga teks tajam, lingkaran warna presisi di tengah sel tanggal, dan teks legenda diukur measureText + ellipsize agar tak terpotong; ikon legenda diambil dari SVG DOM (data-export-icon) lalu dirasterisasi vektor; html2canvas hanya fallback; di ponsel membuka share sheet native, fallback unduh di desktop, batal share = tanpa galat
